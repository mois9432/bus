...
xhr.open('get','https://ptx.transportdata.tw/MOTC/v2/Bus/Route/City/NewTaipei?$top=600&$format=JSON');//
xhr.send(null);
xhr.onload = function () {
    ...
    function showBusWay(items) {
        ...
        for (var i = 0; i < items.length; i++){
            ...
            `<li class="card" title="路線詳情">
                <a href="bus-way.html?Zh_tw=${items[i].RouteName.Zh_tw}" class="busLink">
                    <p class="bus-way">${ items[i].DepartureStopNameZh}-${ items[i].DestinationStopNameZh}</p>
                    <p class="bus-num">${ items[i].RouteName.Zh_tw}</p>
                </a>
            </li>`
        }
